                    ...
                    for artist in sorted_music_tracks(music_tracks).keys():
                        frame:
                            ...
                            for title in sorted(music_tracks[artist].keys()):
                                ...
                                $ locked = _("- locked") if track not in mr.unlocked_playlist() else _("- unlocked")
                                ...
                                if show_locked:
                                    textbutton "{}{}({}){}".format(title," {}".format(locked), length, custom):
                                        if track in mr.unlocked_playlist():
                                            action mr.Play(track),SelectedIf(mr.Play(track))
                                        else:
                                            action Function(single_unlock, track)
    ...
    use game_menu(_("Music Player Settings")):
        vbox:
            ...
            vbox:
                box_wrap True
                style_prefix "check"
                label _("FF and REW time in Seconds: [persistent._music_ff_rew]")
                hbox:
                    style_prefix "slider"
