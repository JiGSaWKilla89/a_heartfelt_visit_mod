...
init 1:#Mod Defaults
    ...
    define gui.jg_mod_version = 'CHP.2.5'
    define gui.mod_update_version = 1
    define gui.dev_mod_page = "a_heartfelt_visit_mod"
    define gui.mod_update_date = "20/10/2024"
    ...
    define gui.developer_name = "Secret Arcade"
    define gui.developer_support = "https://www.patreon.com/SecretArcade"
    ...
    define gui.mod_save_page_input_ypos = -75
    define gui.mod_save_goto_page_xsize = 370
    define gui.mod_save_goto_page_ysize = 50

    define gui.use_custom_caret = False

    define gui.textbox_location = "gui/textbox_backup.png"
...
screen support_mod_developer():
    ...
    button:
        action Hide("support_mod_developer"),Return()
    if closing > 0:
        timer 1 action SetLocalVariable("closing", closing-1) repeat True
    if closing == 0:
        timer .1 action Hide("support_mod_developer"),Return()
    ...
    if not hov:
        key "dismiss" action Hide("support_mod_developer"),Return()
        key "toggle_skip" action Hide("support_mod_developer"),Return()
        key "skip" action Hide("support_mod_developer"),Return()
...
init python:
    ...
    def scene_override(*args):
        renpy.scene(*args)
        renpy.transition(dissolve)
        return
    #config.scene = scene_override#renpy.scene
    ...
    def get_latest_mod():
        ...
        try:
            ...
            if response is not None:
                renpy.write_log("HTTP Error: Received status code {}: {}".format(response.status_code,ce))
            else:
                renpy.write_log("Connection Error: {}".format(ce))
            return "HTTP Error", gui.jg_mod_version

        except requests.Timeout as rto:
            renpy.write_log("Timeout: The request timed out {}".format(rto))
            return "Timeout", gui.jg_mod_version

        except requests.RequestException as e:
            renpy.write_log("Request Error: {}".format(e))
