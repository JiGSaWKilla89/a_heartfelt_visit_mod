...
define LightAndMinimalBackground = AudioCredits(
    "Light and Minimal Background",
    "Coma-Media",
    "sfx/light-and-minimal-background-12036.mp3",
    "Pixabay",
    "https://pixabay.com/music/upbeat-light-and-minimal-background-12036/",
    "",
    "Pixabay License",
    "https://pixabay.com/service/license-summary/",
    unlocked=True
    )
define AGentleBreeze = AudioCredits(
    "A Gentle Breeze",
    "Good_B_Music",
    "sfx/a-gentle-breeze-189206.mp3",
    "Pixabay",
    "https://pixabay.com/music/modern-classical-a-gentle-breeze-189206/",
    "",
    "Pixabay License",
    "https://pixabay.com/service/license-summary/",
    )
define AmbientPianoandStrings = AudioCredits(
    "Ambient Piano and Strings",
    "Good_B_Music",
    "sfx/ambient-piano-and-strings-10711.mp3",
    "Pixabay",
...
define BackgroundMusicInstrumental = AudioCredits(
    "Background Music (Instrumental)",
    "faespencer",
    "sfx/background-music-instrumental-207886.mp3",
    "Pixabay",
    "https://pixabay.com/music/beats-background-music-instrumental-207886/",
    "",
    "Pixabay License",
    "https://pixabay.com/service/license-summary/",
    )
define FlaingPianoMain = AudioCredits(
    "Flaing Piano (Main)",
    "Good_B_Music",
    "sfx/flaing-piano-main-8783.mp3",
    "Pixabay",
    "https://pixabay.com/music/solo-piano-flaing-piano-main-8783/",
    "",
    "Pixabay License",
    "https://pixabay.com/service/license-summary/",
    )
define Flow = AudioCredits(
    "Flow",
    "Loksii",
    "sfx/flow-211881.mp3",
    "Pixabay",
...
define GroovyAmbientFunk = AudioCredits(
    "Groovy Ambient Funk",
    "moodmode",
    "sfx/groovy-ambient-funk-201745.mp3",
    "Pixabay",
...
define MemoriesRemain = AudioCredits(
    "Memories Remain",
    "Onoychenkomusic",
    "sfx/memories-remain-171862.mp3",
    "Pixabay",
    "https://pixabay.com/music/modern-classical-memories-remain-171862/",
    "",
    "Pixabay License",
    "https://pixabay.com/service/license-summary/",
    )
define PerfectBeauty = AudioCredits(
    "Perfect Beauty",
    "Good_B_Music",
    "sfx/perfect-beauty-191271.mp3",
    "Pixabay",
    "https://pixabay.com/music/pulses-perfect-beauty-191271/",
    "",
    "Pixabay License",
    "https://pixabay.com/service/license-summary/",
    )
define RelaxingSoftHandpanMusic = AudioCredits(
    "Relaxing Soft Handpan Music",
    "pixellicious",
    "sfx/relaxing-soft-handpan-music-233448.mp3",
    "Pixabay",
    "https://pixabay.com/es/music/instrumentos-solistas-relaxing-soft-handpan-music-233448/",
    "",
    "Pixabay License",
    "https://pixabay.com/service/license-summary/",
    )
define SadSoulChasingAFeeling = AudioCredits(
    "Sad Soul (Chasing a Feeling)",
    "AlexGrohl",
    "sfx/sad-soul-chasing-a-feeling-185750.mp3",
    "Pixabay",
...
define SimpleStory = AudioCredits(
    "Simple Story |",
    "OpenMusicList",
    "sfx/simple-story-187711.mp3",
    "Pixabay",
    "https://pixabay.com/music/pop-simple-story-187711/",
    "",
    "Pixabay License",
    "https://pixabay.com/service/license-summary/",
    )
define SummerFlings = AudioCredits(
    "Summer Flings",
    "GregFellman",
    "sfx/summer-flings-154778.mp3",
    "Pixabay",
    "https://pixabay.com/music/beats-summer-flings-154778/",
    "",
    "Pixabay License",
    "https://pixabay.com/service/license-summary/",
    )
...
init -1000 python:
    ...
    def music_dur(style_name, size, yalign, st, at):
        ...
        if renpy.music.is_playing(channel="music"):
            try:
                time_duration = get_music_len(renpy.loader.transfn(mr.get_track()), False) or time_duration
            except:
    ...
    #get_music_len(renpy.loader.transfn("sfx/a-gentle-breeze-189206.mp3"))
    def get_music_len(file, readable=True):
        from io import BytesIO
        audio = None

        try:
            track = renpy.file(file)
    ...
    def musicroom_get_duration(self):
        if renpy.music.is_playing(channel="music"):
            try:
                time_duration = get_music_len(renpy.loader.transfn(mr.get_track()), False) or self.duration
            except:
                time_duration = renpy.music.get_duration(
                    channel='music') or time_duration
        else:
    ...
    def prevcurnext(self):
        ...
        try:
            ...
            self._previous_track_text = "{} - {}".format(previous_track_data[0], previous_track_data[1])
            self._current_track_text = "{} - {}".format(current_track_data[0], current_track_data[1])
            self._next_track_text = "{} - {}".format(next_track_data[0], next_track_data[1])
...
define gui.bar_right_color = adjust_brightness("#FB4301", -100)
...
define gui.music_icon_hover_color = adjust_brightness("#FB4301", -100)
...
define gui.musicroom_button_text_hover_color = adjust_brightness("#FB4301", -100)
...
init python:
    ...
    def get_music_files_archive(ignore_list=ignore_list_music):
        """
        
        This Function Gets all the files mainly 'music files'
        files.
        ...
        for file in renpy.list_files():
            if file.startswith(("python-packages", "mod", "tl", "cache", "saves")):
                continue
            if not file in ignore_list:
                if file.endswith((".flac", ".mp3", ".ogg", "opus", ".wav")):
                    print(file)

init -5 python:
    class AudioCredits:
        TrackInfo = []
        def __init__(self, title, artist, filename, link_name, link, description, license, lic_link, mood="", unlocked=False):
            self.title = title
            self.artist = artist
            self.filename = filename
            ...
            AudioCredits.TrackInfo.append(self)
        ...
        def __iter__(self):
            # This returns an iterator that yields the filename as a single-item list
...
init 10 python:
    ...
    def lock_all_tracks():
        for artist, data in music_tracks.items():
            for title, trackdata in data.items():
                track = trackdata.get("musicroom_path","")
                if track:
                    music_tracks[artist][title]["unlocked"] = renpy.seen_audio(track)

    def find_music():
        data = []
        tracks = []

        def run_replacements(item):
            replacements = ("audio/", "music/", "Erotic/", "Pilot/", "Radio/")
        ...
        for i in renpy.list_files():
            ...
            if i.endswith((".mp3", ".ogg", ".flac", ".opus", ".wav")) and not "audio/sound/" in i:
                tracks.append((i, run_replacements(i)))

        for cnt, audio in enumerate(AudioCredits.TrackInfo, 1):
            correction = run_replacements(audio.filename)
            for track in tracks:
                if track[1] == correction:
                    if track[0] != audio.filename:
                        audio.filename = track[0]
                        break
            data.append([audio.title, audio.artist, audio.filename, audio.unlocked])
    ...
    def generate_track_list(lst):
        for title, artist, path, unlocked in lst:
...
init -5 python:
    ...
    def add_to_music_tracks(dictionary, artist, title, path, unlocked=False, full_path=None, length=None, credits={}, custom=False):
        ...
        track_length = length if length else get_music_len(renpy.loader.transfn(path))
        ...
        if not seen_track(path):
            dictionary[artist][title]["unlocked"] = False
    ...
    def add_to_playlist(musicroom):
        for artist, data in music_tracks.items():
            ...
            for track_title, track_data in data.items():
                ...
                unlocked = track_data.get("unlocked", False) and seen_track(path)
...
image settings_outline_idle = Transform(
    'mod/images/settings_outline.png',
    matrixcolor=ColorSingle(gui.music_icon_idle_color),
    xysize=(gui.button_size, gui.button_size),
    align=(0.5,0.5)
)
image settings_outline_hover = Transform(
    'mod/images/settings_outline.png',
    matrixcolor=ColorSingle(gui.music_icon_hover_color),
    xysize=(gui.button_size, gui.button_size),
    align=(0.5,0.5)
)
image settings_outline_selected = Transform(
    'mod/images/settings_outline.png',
    matrixcolor=ColorSingle(gui.music_icon_selected_color),
    xysize=(gui.button_size, gui.button_size),
    align=(0.5,0.5)
)
image settings_outline_insensitive = Transform(
    'mod/images/settings_outline.png',
    matrixcolor=ColorSingle(gui.music_icon_insensitive_color),
    xysize=(gui.button_size, gui.button_size),
    align=(0.5,0.5)
...
image settings_solid_idle = Transform(
    'mod/images/settings_solid.png',
    matrixcolor=ColorSingle(gui.music_icon_idle_color),
    xysize=(gui.button_size, gui.button_size),
    align=(0.5,0.5)
)
image settings_solid_hover = Transform(
    'mod/images/settings_solid.png',
    matrixcolor=ColorSingle(gui.music_icon_hover_color),
    xysize=(gui.button_size, gui.button_size),
    align=(0.5,0.5)
)
image settings_solid_selected = Transform(
    'mod/images/settings_solid.png',
    matrixcolor=ColorSingle(gui.music_icon_selected_color),
    xysize=(gui.button_size, gui.button_size),
    align=(0.5,0.5)
)
image settings_solid_insensitive = Transform(
    'mod/images/settings_solid.png',
    matrixcolor=ColorSingle(gui.music_icon_insensitive_color),
    xysize=(gui.button_size, gui.button_size),
    align=(0.5,0.5)
...
image settings_button_idle = ConditionSwitch(
    "not persistent._use_outline_music_buttons", " settings_outline_idle",
    "persistent._use_outline_music_buttons", " settings_solid_idle",
)

image settings_button_hover = ConditionSwitch(
    "not persistent._use_outline_music_buttons", " settings_outline_hover",
    "persistent._use_outline_music_buttons", " settings_solid_hover",
)

image settings_button_selected = ConditionSwitch(
    "not persistent._use_outline_music_buttons", " settings_outline_selected",
    "persistent._use_outline_music_buttons", " settings_solid_selected",
)

image settings_button_insensitive = ConditionSwitch(
    "not persistent._use_outline_music_buttons", " settings_outline_insensitive",
    "persistent._use_outline_music_buttons", " settings_solid_insensitive",
)