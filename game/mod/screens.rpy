init 5:# Screens
    screen navigation():
        ...
        vbox:
            ...
            if JGSLoadable("replays") and not _in_replay:
                textbutton _("Replays") action ShowMenu("replays"),If(screen_vis,[Hide("color_picker_mr",transition=dissolve),Hide("color_picker_wt",transition=dissolve)])
    ...
    screen main_menu():
        ...
        imagebutton:
            idle "images/patreon_normal.png"
            hover "images/patreon_hover.png"
            action OpenURL("https://patreon.com/SecretArcade")
            at patreonbutton

        imagebutton:
            idle "images/subscribestar_normal.png"
            hover "images/subscribestar_hover.png"
            action OpenURL("https://subscribestar.adult/SecretArcade")
            at subscribestarbutton
    ...
    screen say(who, what, slow_effect=persistent._slow_effect, slow_effect_delay=persistent._effect_delay, always_effect=persistent._always_effect):
        ...
        window:
            ...
            if persistent._textbox_visible and who:
                #add Transform(Frame(gui.textbox_location),
                #    alpha=persistent._textbox_alpha,
                #    xysize=(config.screen_width, gui.textbox_height), yoffset=55)
    ...
    transform button_open(v=.2):
        on idle:
            easein .2 matrixcolor(BrightnessMatrix(0.0))
        on hover:
            easein .2 matrixcolor(BrightnessMatrix(0.2))

    screen button_open():
        zorder 1
        imagebutton auto "images/book_%s.png" xpos 30 ypos 0 focus_mask True action Show("diary") at 